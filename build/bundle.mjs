...
class CircularList {
    ...
    filter(fn) {
        return new this.constructor(this._data.filter((x, i) => fn(x, i, this.neighbourhood(i))));
    }
    [Symbol.iterator]() {
    ...
    *neighbourhoods() {
    ...
    map(fn) {
        return new this.constructor(this._data.map((x, i) => fn(x, i, this.neighbourhood(i))));
    }
    forEach(fn) {
...
function subtract([u, v], [x, y]) {
    return [u - x, v - y];
}
function scale([x, y], c) {
    return [x * c, y * c];
}
function scaleFrom(point, center, factor) {
    return add(scale(subtract(point, center), factor), center);
}
function squaredLength([x, y]) {
    return x * x + y * y;
}
function cross([u, v], [x, y]) {
    return u * y - v * x;
}
function equals([u, v], [x, y]) {
    return u == x && v == y;
}
class ScalarFunction extends CircularList {
    max() { return Math.max(...this._data); }
    min() { return Math.min(...this._data); }
}
function curvatureSign(x) {
    ...
    return 0.5 * cross(twiceDisplacement, laplacian) * Math.pow(dr2, (-3 / 2));
}
function curvature(x) {
    return Math.abs(curvatureSign(x));
}
class Curve extends CircularList {
    curvature() {
        return new ScalarFunction(this.map((p, i, nbhd) => curvature(nbhd)));
    }
...
function remesh(cu, seglength) {
    ...
    for (let i = 0; i < cu.length; i++) {
        ...
        if (dr2 > 4 * seglength * seglength) {
            ...
            cu.splice(1 + i, 0, add(a, scale(displacement, seglength * Math.pow(dr2, (-1 / 2)))));
        }
        else if (cu.length > 4 && dr2 * 4 < Math.pow(seglength, 2)) {
 ...
 */
function renderClosedCurve(curve, ctx, { colorFunction = constant('black') } = {}) {
    ctx.save();
 ...
 */
function renderPath(path, ctx, join_opacity = 0) {
    ctx.save();
...
var bind = unwrapExports(bindDecorator);
...
const curvatureColor = (k) => colorInterpolate(['black', 'red'])(k * 100);
let CSFApp = class CSFApp extends LitElement {
    constructor() {
        super(...arguments);
        this.touchPaths = new Map();
        this.curves = [];
        this.seglength = 5;
        this.bufferWidth = 300;
        this.bufferHeight = 200;
        this.preserveArea = false;
        this.colorCode = true;
        this.paused = false;
    }
    get ctx() {
    ...
    touchMove(e) {
        ...
        if (!lastPos || d2(pos, lastPos) > Math.pow(this.seglength, 2))
            path.push(pos);
    }
    touchEnd(e) {
        const path = this.touchPaths.get(e.identifier);
        if (!path)
            return;
        let p = path[0];
        while (d2(p, path[path.length - 1]) > Math.pow(this.seglength, 2)) {
            let q = path[path.length - 1];
            let d = subtract(p, q);
            let l = Math.pow(squaredLength(d), (1 / 2));
    ...
    tick() {
        ...
        this.curves = this.curves.filter(cu => cu.length >= 5 && cu.curvature().max() < 5000);
        ...
        for (let [j, cu] of this.curves.entries()) {
            ...
            clean(cu);
            const colorFunction = this.colorCode
...
__decorate([
    query('canvas')
], CSFApp.prototype, "canvas", void 0);
__decorate([
    property({ type: Number })
], CSFApp.prototype, "bufferWidth", void 0);
__decorate([
    property({ type: Number })
], CSFApp.prototype, "bufferHeight", void 0);
__decorate([
    property({ type: Boolean })
], CSFApp.prototype, "preserveArea", void 0);
__decorate([
    property({ type: Boolean })
], CSFApp.prototype, "colorCode", void 0);
__decorate([
    property({ type: Boolean })
], CSFApp.prototype, "paused", void 0);
__decorate([
    bind
], CSFApp.prototype, "startFlow", null);
__decorate([
    bind
], CSFApp.prototype, "resize", null);
__decorate([
    bind
], CSFApp.prototype, "touchStart", null);
__decorate([
    bind
], CSFApp.prototype, "touchMove", null);
__decorate([
    bind
], CSFApp.prototype, "touchEnd", null);
__decorate([
    bind
], CSFApp.prototype, "tick", null);
CSFApp = __decorate([
    customElement('csf-app')
], CSFApp);
function demoCurve(canvas) {
